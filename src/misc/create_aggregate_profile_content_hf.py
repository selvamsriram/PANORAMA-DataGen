...
def process_panorama_record(record: dict) -> dict:
    ...
    if (locale != "en_US" or age > 35 or age < 20):
        #logging.info(f"Record {unique_id} excluded due to locale or age. Locale: {locale}, Age: {age}")
        return {
            "Unique ID": unique_id,
            "ConcatContentPairs": ""
        }
    ...
    concat_content_pairs = "\n".join(result_strings)
    logging.debug(f"ConcatContentPairs: {concat_content_pairs} for record: {unique_id}")
    return {
        "Unique ID": unique_id,
        "ConcatContentPairs": concat_content_pairs
    }
